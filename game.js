...
k.onUpdate(() => {
    ...
    // Update all moving objects speed
    k.get("enemy").forEach(obj => {
        obj.move = k.vec2(-gameState.speed, 0);
    });
    k.get("obstacle").forEach(obj => {
        obj.move = k.vec2(-gameState.speed, 0);
    });
    k.get("bullet").forEach(obj => {
        obj.move = k.vec2(-(gameState.speed + 200), 0);
    });
    ...
    k.get("building").forEach(obj => {
        obj.move = k.vec2(-gameState.speed * 0.3, 0);
    });
...
// Game scene setup is already done above
